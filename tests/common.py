...
========================================"""
    ...
    def assertMetric(self, metric_name, value=None, tags=None, count=None, at_least=None):
    ...
    def assertMetricTagPrefix(self, metric_name, tag_prefix, count=None, at_least=None):
        log.debug("Looking for a tag starting with `{0}:` on metric {1}".format(tag, metric_name))
        if count is not None:
            log.debug(" * should have exactly {0} data points".format(count))
        if at_least is not None:
            if count is not None:
    ...
    def assertMetricTag(self, metric_name, tag, count=None, at_least=None):
    ...
    def assertServiceCheck(self, service_check_name, status=None, tags=None, count=None, at_least=None):
